 ...
 */
class adsanity_random_ad_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			'classname'                   => 'adsanity-random',
			'description'                 => __( 'Display a single random ad from an Ad Group.', 'adsanity' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct(
			'adsanity-random',
			sprintf( 'AdSanity - %s', __( 'Random Ad', 'adsanity' ) ),
			$widget_ops
		);
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( array_key_exists( 'align', $new_instance ) ) {

			$updated_instance['align'] = $new_instance['align'];

		}

		if ( array_key_exists( 'max-width', $new_instance ) && $new_instance['max-width'] ) {

			$updated_instance['max_width'] = $new_instance['max-width'];
	...
	function form( $instance ) {
		...
		echo '<div class="adsanity-random-options">';
		echo sprintf( '<label for="%s">%s</label> ',
...
/**
 * Register the Random Ad Group widget with WordPress.
 *
 * @return void
 */
function adsanity_register_random_ad_widget() {

	register_widget( 'adsanity_random_ad_Widget' );

}

add_action( 'widgets_init', 'adsanity_register_random_ad_widget' );
