	...
	*/
	...
	(function ($) {
	  $(document).ready(function () {
	    ...
	    function setNavDropdownItem() {
	      ...
	      var navDropdownItem = $('#main-menu').find("[data-bs-toggle]"); // 768 = $screen-md, 992 = $screen-lg
	  ...
	  $(".gform_wrapper").on("focus", '.ginput_container:not(.ginput_container_fileupload) > input[type!="hidden"], .ginput_container > select', function () {
	    $(this).closest(".gfield:not(.gfield_multi_time)").addClass("has-focus-or-value");
	...
	(function ($) {
	  function load_more_func(e, el, item) {
	    e.preventDefault(); // disable load more button for accidental double clicks

	    el.prop('disabled', true); // Pass in any necessary attributes
	    ...
	    var buttonText = el.text(); // console.log( max_page );

	    console.log(data);
	    $.ajax({
	      url: data.site + '/wp-admin/admin-ajax.php',
	      // AJAX handler
	      data: data,
	      type: 'POST',
	      beforeSend: function () {
	      ...
	      success: function (html) {
	        ...
	        if (html) {
	          // console.log( html );
	          item.append(html);
	          button.attr('current-page', ++current_page); // button.text( '' );
	          ...
	          var myEle = document.getElementById("renoExclusionArray"); // var loadRenoButton = document.getElementById("loadMoreRenovators");
	          ...
	          if (myEle) {
	            ...
	            myEle.remove(); // loadRenoButton.setAttribute("exclude-posts", myEleValue);
